...
class SigLipVisionTower(nn.Module):
    ...
    def forward(self, images):
        if type(images) is list:
            ...
            image_features = []  
            # import numpy as np
            indexes = torch.zeros(len(images)+1, dtype=int)
            for i, image in enumerate(images):  
                image_feature = self.vision_tower(image.to(device=self.device, dtype=self.dtype), output_hidden_states=True)  
                image_feature = image_feature.hidden_states[-1].to(image.dtype)  
                assert image_feature.shape[-2] == 729  
                image_features.append(image_feature)  
                indexes[i+1] = indexes[i] + image_feature.shape[0]  
            image_features = torch.cat(image_features, dim=0) if image_features else None
            return image_features, indexes
            ...
            #     indexes.append(image_features.shape[0])
            # return image_features, indexes
